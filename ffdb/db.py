...
def column_for_stat(stat_name: str) -> str:
    return f"stat_{stat_name}" if stat_name in PLAYER_GAME_FIXED_COLUMNS else stat_name
...
def ensure_stat_columns(conn: sqlite3.Connection, stat_names: Iterable[str]) -> dict[str, str]:
    """Add a REAL column for each unseen stat key. Returns stat -> column mapping."""
    present = existing_columns(conn, "player_games")
    mapping: dict[str, str] = {}
    for stat_name in stat_names:
        column = column_for_stat(stat_name)
        mapping[stat_name] = column
        if column not in present:
            conn.execute(f'ALTER TABLE player_games ADD COLUMN "{column}" REAL')
            present.add(column)
            log.info("added stat column %s", column)
        conn.execute(
            "INSERT INTO stat_catalog (stat_name, column_name, first_seen) VALUES (?, ?, ?) "
            "ON CONFLICT(stat_name) DO NOTHING",
            (stat_name, column, now_iso()),
        )
    conn.commit()
    return mapping
...
def upsert_player_games(conn: sqlite3.Connection, rows: Iterable[dict]) -> int:
    ...
    mapping = ensure_stat_columns(conn, stat_names)
    ...
    for row in rows:
        ...
        _upsert(conn, "player_games", record, ("athlete_id", "event_id"))
        count += 1
    conn.commit()
    return count
